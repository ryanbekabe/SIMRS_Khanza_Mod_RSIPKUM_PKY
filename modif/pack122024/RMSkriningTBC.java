...
public final class RMSkriningTBC extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        MnSkriningTBC1 = new javax.swing.JMenuItem();
        ...
        MnSkriningTBC.setText("Formulir Skrining TBC - Petugas");
        ...
        MnSkriningTBC1.setBackground(new java.awt.Color(255, 255, 254));
        MnSkriningTBC1.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnSkriningTBC1.setForeground(new java.awt.Color(50, 50, 50));
        MnSkriningTBC1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnSkriningTBC1.setText("Formulir Skrining TBC - Dokter");
        MnSkriningTBC1.setName("MnSkriningTBC1"); // NOI18N
        MnSkriningTBC1.setPreferredSize(new java.awt.Dimension(200, 26));
        MnSkriningTBC1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnSkriningTBC1ActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MnSkriningTBC1);
...
//            param.put("finger","https://verifqr.rsipalangkaraya.co.id/verifqr/?id="+EnkripsiAES.encrypt("{'x':'skt','t':'"+NoSurat.getText()+"'}"));
            Valid.MyReportqry("rptFormulirSkriningTBC.jasper","report","::[ Formulir Skrining TBC - Petugas ]::",
    ...
    private void MnSkriningTBC1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnSkriningTBC1ActionPerformed
        if(tbObat.getSelectedRow()>-1){
            Map<String, Object> param = new HashMap<>();
            param.put("namars",akses.getnamars());
            param.put("alamatrs",akses.getalamatrs());
            ...
            param.put("kontakrs",akses.getkontakrs());
            param.put("emailrs",akses.getemailrs());   
            param.put("sipdokter",Sequel.cariIsi("SELECT dokter.no_ijn_praktek FROM skrining_tbc INNER JOIN petugas ON petugas.nip=skrining_tbc.nip INNER JOIN dokter ON dokter.kd_dokter=petugas.nip WHERE skrining_tbc.no_rawat=?",TNoRw.getText()));
//            SELECT dokter.no_ijn_praktek FROM skrining_tbc INNER JOIN petugas ON petugas.nip=skrining_tbc.nip INNER JOIN dokter ON dokter.kd_dokter=petugas.nip WHERE skrining_tbc.no_rawat= '2024/12/02/000168';
//            param.put("logo",Sequel.cariGambar("select setting.logo from setting")); 
            param.put("logo",Sequel.cariGambar("select gambar.kopsurat from gambar"));
            finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",tbObat.getValueAt(tbObat.getSelectedRow(),5).toString());
            param.put("finger","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+tbObat.getValueAt(tbObat.getSelectedRow(),6).toString()+"\nID "+(finger.equals("")?tbObat.getValueAt(tbObat.getSelectedRow(),5).toString():finger)+"\n"+Tanggal.getSelectedItem()); 
//            param.put("finger","https://verifqr.rsipalangkaraya.co.id/verifqr/?id="+EnkripsiAES.encrypt("{'x':'skt','t':'"+NoSurat.getText()+"'}"));
            Valid.MyReportqry("rptFormulirSkriningTBCD.jasper","report","::[ Formulir Skrining TBC - Dokter]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,pasien.jk,skrining_tbc.nip,petugas.nama,dokter.kd_dokter,dokter.nm_dokter,dokter.no_ijn_praktek,skrining_tbc.tanggal,"+
                    "skrining_tbc.berat_badan,skrining_tbc.tinggi_badan,skrining_tbc.imt,skrining_tbc.kasifikasi_imt,skrining_tbc.lingkar_pinggang,skrining_tbc.risiko_lingkar_pinggang,"+
                    "skrining_tbc.riwayat_kontak_tbc,skrining_tbc.jenis_kontak_tbc,"+
                    "skrining_tbc.faktor_resiko_pernah_terdiagnosa_tbc,skrining_tbc.keterangan_pernah_terdiagnosa,"+
                    "skrining_tbc.faktor_resiko_pernah_berobat_tbc,skrining_tbc.faktor_resiko_malnutrisi,skrining_tbc.faktor_resiko_merokok,skrining_tbc.faktor_resiko_riwayat_dm,"+
                    ...
                    "skrining_tbc.abnormalitas_tbc,skrining_tbc.gejala_tbc_batuk,skrining_tbc.gejala_tbc_bb_turun,skrining_tbc.gejala_tbc_demam,skrining_tbc.gejala_tbc_berkeringat_malam_hari,skrining_tbc.keterangan_gejala_penyakit_lain,"+
                    "skrining_tbc.kesimpulan_skrining,skrining_tbc.keterangan_hasil_skrining from skrining_tbc inner join reg_periksa on skrining_tbc.no_rawat=reg_periksa.no_rawat "+
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join petugas on skrining_tbc.nip=petugas.nip inner join dokter on dokter.kd_dokter=petugas.nip "+
                    "where reg_periksa.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"'",param);
        }
    }//GEN-LAST:event_MnSkriningTBC1ActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem MnSkriningTBC1;
